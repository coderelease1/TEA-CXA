...
class MMEnv(ABC):
    ...
    def step(self, responses, processor, image_data: List[List[Image.Image]], image_path, extra_info):
        tokenizer = processor.tokenizer
        print("start to the env step", file=sys.stderr, flush=True)
        cur_actions, tool_results = self.tool_manager.execute_actions(responses=responses, image_data=image_data, image_path=image_path, extra_info=extra_info)
        ...
        for action, tool_result in zip(cur_actions, tool_results):
            ...
            if action == 'answer':
                temp_next_obs, temp_done, temp_valid_action, temp_is_tool, temp_image_data, temp_valid_tool, temp_raw_prompt = '', True, 1, 0, None, 0, None #0, 1
            elif action == 'error':
