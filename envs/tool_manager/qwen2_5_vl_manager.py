...
class Qwen25VLManager(ToolManager):    
    ...
    def _build_tools(self):
        ...
        if config_path is not None:
            ...
            #remove image_path_list, extra_info
            for function in self.functions:
                del function['parameters']['properties']['image_path_list']
                function['parameters']['required'].pop(0)
                del function['parameters']['properties']['extra_info']
                function['parameters']['required'].pop(-1)
        else:
            print("The config_path is None!")
            self.functions = []

    async def execute_all_tools(self, actions, tool_list, image_data: List[List[Image.Image]], image_path, extra_info):
        """异步并行执行所有工具列表
        ...
        # 并行执行每个工具列表
        tasks = []
        for temp_action, temp_tool_list, temp_image_data, temp_image_path, temp_extra_info in zip(actions, tool_list, image_data, image_path, extra_info):
            tasks.append(self._execute_tool_batch(temp_action, temp_tool_list, temp_image_data, temp_image_path, temp_extra_info))
        
        results = await asyncio.gather(*tasks)
        
        return results
        
    async def _execute_tool_batch(self, action, tools, image_data: List[Image.Image], image_path, extra_info):
        """异步并行执行一批工具
        ...
        Returns:
            工具执行结果的列表
        """        
        async def execute_single_tool(tool, image_data:Image.Image, image_path, extra_info):
            ...
            if tool_instance is not None:
                try:
                    ...
                    args["image_path_list"] = image_path.tolist()
                ...
                if type(args) is dict:
                    try:
                        ...
                        if isinstance(image_result, Image.Image):
                            text_result = [
                                {"type": "text", "text": f"Execute the tool {tool['name']} succeeded. The args are: {original_args}. The image result is: "},
                                {"type": "image"}
                            ]
                            return (text_result, image_result)
                        elif isinstance(image_result, str):
                            text_result = [
                                {"type": "text", "text": f"Execute the tool {tool['name']}. The result is: {image_result}\n"}#The args are: {original_args}. 
                            ]
                        ...
                        for key, value in args.items():
                            s += f"{key}: {value} (type: {type(value).__name__})"          
                        result = f"Execute the tool {tool['name']} failed. The original args are: {original_args}. Error message: {str(e)}.\n{s}\n"                        
                else:
                    result = f"Execute the tool {tool['name']} failed. Arguments must be a JSON object, got: {type(args).__name__} with value: {args}"
            else:
                result = f"Tool name is wrong!" #"Failed to find the tool {tool['name']} in the tool map."
        ...
        if action == 'answer':
            ...
            tools = [{"type": "text", "text": tools}]
            results = [({'role': 'assistant', 'content': tools}, None)] #[({'role': 'assistant', 'content': tools}, None)]
        elif action == 'error':
            ...
            message = [{"type": "text", "text": tools}]
            results = [({'role': 'assistant', 'content': message}, None)]
        elif action == 'actions':
            # 'tools' is the list of the 'Tool' instances
            tasks = [execute_single_tool(temp_tool, image_data[-1], image_path, extra_info) for temp_tool in tools]
            tool_results = await asyncio.gather(*tasks)

            #results = [({'role': 'tool', 'content': temp_tool_result[0]}, temp_tool_result[1]) for temp_tool_result in tool_results]
            results = [( {'role': 'tool', 'content': [msg[0] for msg, _ in tool_results]}, None )]
    ...
    def execute_actions(self, responses: List[str], image_data: List[List[Image.Image]], image_path, extra_info):
        ...
        try:
            tool_results = asyncio.run(self.execute_all_tools(actions, tools, image_data, image_path, extra_info))
        except RuntimeError:
            # 如果事件循环已经在运行，则获取当前循环
            loop = asyncio.get_event_loop()
            tool_results = loop.run_until_complete(self.execute_all_tools(actions, tools, image_data, image_path, extra_info))
        #print(tool_results)
    ...
    def parse_response(self, response_content: str) -> Optional[Dict[str, Any]]:
        ...
        # 提取tools
        action, tools = self.parse_tools(response_content)
        return action, tools
        if type(tools) == list:
...
If I want to call a tool function, I should put the query between <tool_call> and </tool_call>. \
If I want to give the final answer, I should put the final answer choice (A/B) between <answer> and </answer>. Let me try again.\n'
        reflect_msg_function_call = '\nMy previous action is invalid. \
For each function call, I should return a json object with function name and arguments within <tool_call></tool_call> XML tags:\
<tool_call>{"name": <function-name>, "arguments": <args-json-object>}</tool_call>'
        ...
        if i < 0:
            return 'error', reflect_msg_tag#response
        ...
        for txt in tool_call_list[1:]:
            ...
            if '</tool_call>' not in txt:
                ...
                continue  # Stop after the first tool call (even if incomplete)
            else:
                ...
                try:
                    ...
                    return 'error', reflect_msg_function_call
                    break  # Stop after the first error
        
        if len(parsed_tools)==0:
            return 'error', reflect_msg_tag
        return 'actions', parsed_tools
