...
class MMToolUtils(ToolUtils):
    ...
    def postprocess_output(self, output: DataProto, step: int):
        ...
        if self.loop_cnt == 0:
            ...
            self.image_path_list = [i for i in output.non_tensor_batch["image_path"]]
            self.image_extrainfo_list = [i for i in output.non_tensor_batch["x-ray info"]]
        ...
        infos_str, dones, _, _, new_image_data, raw_prompt, multi_modal_data, valid_tool = self.env_object.step(
            responses=responses_str, processor=self.processor, image_data=[self.image_list[idx] for idx in batch_idxs], image_path=[self.image_path_list[idx] for idx in batch_idxs], extra_info=[self.image_extrainfo_list[idx] for idx in batch_idxs]
        )
        # breakpoint()
        for idx, batch_idx in enumerate(batch_idxs):
        ...
        for idx, batch_idx in enumerate(batch_idxs):
            if not dones[idx]:
                ...
                if len(promt_token) > self.config_prompt_length + self.config_response_length:
                    next_sample_idx.pop()
                    self.loop_responses_token[batch_idx].pop()
                    self.loop_raw_responses_token[batch_idx].pop()
                ...
                next_image_path.append(self.image_path_list[batch_idx])
        
        if len(next_prompt_token) == 0:
            return 
        
        # left pad
        max_len = max(next_prompt_length)#(max(next_prompt_length) + self.config_prompt_length)
        ...
        #next_image_data = np.array([{"image": img, "image_path": img_path} for img,img_path in zip(next_image_data,next_image_path)], dtype=object)
        next_image_data = np.array([{"image": img} for img,img_path in zip(next_image_data,next_image_path)], dtype=object)
        ...
        next_data.meta_info['do_sample'] = True #False # step > 0 does not do sample
        self.loop_cnt += 1
        return next_data
