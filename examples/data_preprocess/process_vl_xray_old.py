...
instruction_following = (
    r"You are a medical imaging expert. Given a question and images, answer the question."
    r"Any time you receive new information, you should reason step by step inside the <think> and </think> XML tag. "
    r"Afterwards, you can either choose to call one tool function to complement your reasoning or directly provide the final answer choice (A/B/C/D/E/F). "
    "The final answer choice (a single letter) should be wrapped with <answer></answer> XML tag. For example, <answer>A</answer>\n"
)
# Copyright 2024 Bytedance Ltd. and/or its affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dir", default="/data0/Users/zahuai/RL-Factory/chestagentbench")
    #parser.add_argument("--hdfs_dir", default=None)

    args = parser.parse_args()

    # data_source = "/mnt/dolphinfs/hdd_pool/docker/share/jjw/visual_tool/huggingface.co/datasets/hiyouga/geometry3k"
    data_source = "/data0/Users/zahuai/RL-Factory/chestagentbench/train.jsonl"
    data_source1 = "/data0/Users/zahuai/RL-Factory/chestagentbench/test.jsonl"
    # dataset = datasets.load_dataset(data_source)
    # breakpoint()
    train_dataset = datasets.load_dataset("json",data_files=data_source)["train"] 
    # breakpoint()
    test_dataset = datasets.load_dataset("json",data_files=data_source1)["train"] 
    # breakpoint()
...
'{"type": "function", "function": {"name": "vision-classification", "description": "Tool that classifies chest X-ray images for multiple pathologies.\
...
"parameters": Do not provide any parameters.}}\n</tools>\n'
                                'For each function call, return a json object with function name and arguments within <tool_call></tool_call> XML tags:\n'
                                '<tool_call>\n{"name": <function-name>, "arguments": <args-json-object>}\n</tool_call>\nThe tool will return its response between <tool_response> and </tool_response> XML tags. ' #The tool response will be wrapped with <tool_response></tool_response> XML tags.                              
    ...
    #train_dataset.to_json(os.path.join(local_dir, "train_processed.jsonl"), orient="records", lines=True, force_ascii=False)
    #test_dataset.to_json(os.path.join(local_dir, "test_processed.jsonl"), orient="records", lines=True, force_ascii=False)

    save_dataset_as_jsonl_no_escaped_slash(train_dataset, os.path.join(local_dir, "train_processed.jsonl"))
    save_dataset_as_jsonl_no_escaped_slash(test_dataset, os.path.join(local_dir, "test_processed.jsonl"))